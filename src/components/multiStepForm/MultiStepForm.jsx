...
import FontSelection from '@/components/multiStepForm/steps/FontSelection';
...
const MultiStepForm = ({ onSubmit: onSubmitExternal, mode = 'register' }) => {
  ...
  useEffect(() => {
    if (!isInitialized.current) {
      resetForm(mode);
      setTotalSteps(mode === 'dashboard' ? 11 : 12);
  ...
  const renderStepContent = () => {
    if (mode === 'dashboard') {
      switch (currentStep) {
        ...
        case 7:
          return <FontSelection onNext={() => setCurrentStep(8)} />;
        case 8:
    ...
    switch (currentStep) {
      ...
      case 8:
        return <FontSelection onNext={() => setCurrentStep(9)} />;
      case 9:
        return <ColorPaletteSelection onNext={() => setCurrentStep(10)} />;
      case 10:
        return <StepTiming onNext={() => setCurrentStep(11)} />;
      case 11:
        return <StepBudget onNext={() => setCurrentStep(12)} />;
      case 12:
        return <StepMission onSubmit={handleSubmit} isSubmitting={isSubmitting} />;
      default:
        setCurrentStep(1);
