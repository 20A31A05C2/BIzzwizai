...
import { Save, Plus, Minus, Copy, Trash2, RefreshCw, Heart } from 'lucide-react';
...
const ColorPaletteSelection = () => {
  ...
  const handleSave = () => {
    ...
    alert(t('colorPaletteSelection.saveAlert')); // Translated alert
  };

  // Copy individual color
  const copyColor = (color) => {
    navigator.clipboard.writeText(color);
    alert(t('colorPaletteSelection.copyAlert', { color })); // Translated alert with dynamic color
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="mt-auto pt-6 flex justify-between items-center">
        ...
        <Button
          onClick={handleSave}
          className="flex items-center gap-2 bg-bizzwiz-accent text-bizzwiz-text-main hover:bg-bizzwiz-accent-dark"
          aria-label={t('colorPaletteSelection.saveLabel')}
        >
          <Save className="w-4 h-4" />
          {t('colorPaletteSelection.saveButton')}
        </Button>
