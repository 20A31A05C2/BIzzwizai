        ...
        <div className="w-full max-w-7xl flex flex-col xl:flex-row gap-6 xl:gap-12">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
              ...
              <div>
                <Label htmlFor="userCompany" className="text-bizzwiz-text-alt text-sm font-medium">
                  {t('register.companyLabel')} <span className="text-gray-400">{t('register.companyOptional')}</span>
                </Label>
                <div className="relative mt-2">
                  <Input
                    id="userCompany"
                    name="userCompany"
                    value={formData?.userCompany ?? defaultFormData.userCompany}
                    onChange={handleChange}
                    onBlur={handleBlur}
                    placeholder={t('register.companyPlaceholder')}
                    className={`w-full bg-white/10 border-bizzwiz-electric-cyan/30 text-white placeholder-bizzwiz-comet-tail pl-12 pr-4 py-3 rounded-lg focus:bg-white/15 focus:outline-none focus:ring-2 focus:ring-bizzwiz-electric-cyan/50 transition-all duration-300 ${
                      errors.userCompany ? 'border-red-400 focus:ring-red-400/50' : 'focus:border-bizzwiz-electric-cyan'
                    }`}
                    disabled={isSubmitting}
                  />
                  <Building size={18} className="absolute left-4 top-1/2 transform -translate-y-1/2 text-bizzwiz-electric-cyan pointer-events-none" />
                </div>
                {touched.userCompany && errors.userCompany && (
                  <div className="flex items-center mt-2 text-red-400 text-xs bg-red-400/10 px-2 py-1 rounded border border-red-400/20">
                    <XCircle size={14} className="mr-2 flex-shrink-0" />
                    <span>{errors.userCompany}</span>
                  </div>
                )}
              </div>
