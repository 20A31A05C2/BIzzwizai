...
//       bgColor: "bg-blue-50",
//       stepLabels: [
//         "Create Project", "Basics", "Fundamentals", "Concepts", "Practice 1", 
...
//   const stepRoutes = {
//     1: [
//       '/create-project', // Step 1 - Create Project
//       '/level1/basics', 
//       '/level1/fundamentals', 
//       '/level1/concepts',
//       '/level1/practice1', 
//       '/level1/intermediate', 
//       '/level1/advanced-concepts', 
//       '/level1/practice2',
//       '/level1/mastery', 
//       '/level1/final-practice', 
//       '/level1/assessment', 
//       '/level1/completion'
//     ],
//     2: [
//       '/level2/strategy-basics', 
//       '/level2/strategy-advanced', 
//       '/level2/planning', 
//       '/level2/execution',
//       '/level2/analysis', 
//       '/level2/optimization', 
...
//       '/level3/application', 
//       '/level3/expert', 
//       '/level3/advanced-practice',
//       '/level3/final-challenge', 
//       '/level3/mastery'
//     ],
//     4: [
//       '/level4/theory', 
//       '/level4/advanced-theory', 
//       '/level4/expert-practice', 
//       '/level4/final-mastery'
//     ]
//   };
...
//       // For better user experience, use this:
//       window.location.assign(route);
      
//       // Or if you have React Router's useNavigate hook:
//       // navigate(route);
//     }
...
//               <div className="space-y-3">
//                 <button
//                   onClick={() => window.location.assign('/create-project')}
...
import { Star, Trophy, Lock, CheckCircle, ChevronRight } from "lucide-react";
...
const Guideline = () => {
  const { setCurrentStep, resetForm, formData, setFormData } = useFormContext();
  ...
  const [currentStep, setCurrentStepState] = useState({ level: 1, step: 3 });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setCompletedSteps((prev) => ({
          ...prev,
          1: [...prev[1], 11, 12], // Mark Mission and Completion as done
        }));
        setUserXP((prev) => prev + 20); // Award XP for completion
        resetForm("register");
      ...
      setTimeout(() => {
        ...
        setShowForm(false);
        setCurrentStepState({ level: 1, step: 12 }); // Move to Completion
      }, 1000);
    }
  };
  ...
  const handleStepClick = (step, level) => {
    if (!isStepAccessible(step, level)) return;
    if (level === 1 && step <= 11) {
      resetForm("register"); // Reset form when starting Level 1 steps
      setCurrentStep(step); // Set MultiStepForm step
      setShowForm(true); // Show the form
      setCurrentStepState({ level, step }); // Update UI state
    } else {
      setShowForm(false);
      setCurrentStepState({ level, step });
      navigate(stepRoutes[level][step - 1]);
    }
    ...
    >
      <div className="relative mb-8">
        <div className="w-16 h-16 md:w-20 md:h-20 border-4 border-gray-200 rounded-full animate-spin">
          <div className="w-full h-full border-4 border-t-blue-500 border-r-transparent border-b-transparent border-l-transparent rounded-full animate-spin"></div>
        </div>
      ...
      >
        Setting up your account and preparing your workspace...
      </motion.p>
      <motion.div
        ...
        <div className="mt-3 text-center max-w-20">
          <div
            className={`text-xs font-semibold ${
              completed ? "text-green-600" : accessible ? "text-gray-700" : "text-gray-400"
            }`}
          >
            {levelData.stepLabels[step - 1] || `Step ${step}`}
          </div>
          {completed && (
            <div className="text-xs text-green-600 mt-1 animate-pulse">+10 XP</div>
          )}
        </div>
      </div>
    );
  ...
  const renderLevelPath = (levelNum, levelData) => {
        ...
        >
          ...
          <div className="flex flex-col items-center space-y-8 opacity-50">
            {steps.slice(0, 3).map((step) => (
              <div
                key={step}
                className="w-12 h-12 bg-gray-300 rounded-full border-4 border-gray-400 flex items-center justify-center"
              >
                <Lock className="w-4 h-4 text-gray-500" />
              </div>
            ))}
    ...
    return (
      <div
        key={`level-${levelNum}`}
        className="relative mb-16 transition-all duration-500 bg-black"
        style={{
          animation: "slideInUp 0.6s ease-out",
          animationDelay: `${level * 0.2}s`,
          animationFillMode: "both",
        ...
        <div className="relative max-w-md mx-auto">
          ...
          {completedCount === levelData.steps && (
            <div className="text-center py-8 animate-bounce">
              <div
                className={`inline-flex items-center space-x-2 bg-gradient-to-r ${levelData.color} text-white px-6 py-3 rounded-full font-bold shadow-lg`}
              >
                <Trophy className="w-5 h-5" />
                <span>Level {levelNum} Complete!</span>
              </div>
              ...
              <div key={levelNum} className="space-y-2">
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-2">
                    <span className="text-sm font-bold text-gray-800">Level {levelNum}: {levelData.title}</span>
                    {unlocked ? (
                  ...
                  <span className="text-sm text-gray-600">{completedCount}/{total}</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`bg-gradient-to-r ${levelData.color} h-2 rounded-full transition-all duration-500`}
                ...
                <div className="space-y-1">
                  {levelData.stepLabels.map((label, index) => {
                    const step = index + 1;
                    const completed = isStepCompleted(step, level);
                    const accessible = isStepAccessible(step, level);
                    const isCurrent = currentStep.level === level && currentStep.step === step;
                    ...
                    return (
                      <button
                        key={step}
                        className={`w-full flex items-center justify-between text-sm py-2 px-3 rounded-md transition-all duration-200 ${
                          completed
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 bg-black">
        {showForm && currentStep.level === 1 && currentStep.step <= 11 ? (
          <div className="w-full max-w-3xl mx-auto flex flex-col items-center px-4 sm:px-6 lg:px-8">
            <Header />
            {!isReloading && (
              <div className="w-full">
                <ProgressBar currentStep={currentStep.step} totalSteps={11} />
              </div>
            )}
            <div className={`w-full ${!isReloading ? "mt-8" : "mt-4"}`}>
              <AnimatePresence mode="wait">
                {isReloading ? (
                  ...
                  <StepCard key={currentStep.step}>{renderStepContent()}</StepCard>
                )}
              </AnimatePresence>
            </div>
          </div>
